...
function moveEnemies(delta) {
    ...
    const detectionAngle = level >= 5 ? Math.PI / 2 : level >= 3 ? Math.PI / 0.2 : Math.PI / 4;
    ...
    enemies.forEach(enemy => {
        ...
        if (level >= 3 && player) {
            ...
            const dotProduct = dot;
            if (dotProduct > Math.cos(detectionAngle) && hasLineOfSight(enemy, player)) {
...
function shootFireball() {
    ...
    fireball.userData = {
        velocity: offset.clone().normalize().multiplyScalar(10),
        lifetime: 2
    };
    scene.add(fireball);
...
function setupTouchControls() {
    ...
    joystickWrapper.addEventListener('touchmove', (e) => {
        if (joystickActive && !isPaused) {
            ...
            const maxDistance = 30; // Radio del movimiento del joystick
            ...
            if (useDeviceOrientation) {
                if (typeof DeviceOrientationEvent.requestPermission === 'function') {
                    DeviceOrientationEvent.requestPermission()
                        ...
                        .catch(err => {
                            console.error('Error al solicitar permiso de orientación:', err);
                            useDeviceOrientation = false;
                            deviceControls.enabled = false;
                            orientationToggle.textContent = 'Activar Orientación';
                            alert('Error al solicitar permiso de orientación.');
                        });
                } else {
                    deviceControls.connect();
                }
    ...
    window.addEventListener('mousemove', e => {
        if (!thirdPerson && !isPaused && isLeftMousePressed) {
            ...
            firstPersonCameraPitch -= e.movementY * mouseSensitivity;
            firstPersonCameraPitch = Math.max(-maxPitch, Math.min(maxPitch, firstPersonCameraPitch)); // Limitar inclinación
        }
    });
        ...
        if (!isPaused && controls && !isTransitioning && level !== 5) {
            ...
            if (thirdPerson) {
                ...
                if (player) {
                    ...
                    const lookAtPoint = new THREE.Vector3(
                        player.position.x - Math.sin(firstPersonCameraYaw) * 5,
                        player.position.y + firstPersonCameraHeight,
                        player.position.z - Math.cos(firstPersonCameraYaw) * 5
                    );
                    const lookAtMatrix = new THREE.Matrix4().lookAt(
                        targetPosition,
...
function updateLevelParameters() {
    ...
    timeLimit = level === 5 ? 40 : 60 - (level - 1) * 10;
...
function resetGame() {
    ...
    firstPersonCameraPitch = 0; // Resetear inclinación
    ...
    if (player && !gameWon && !gameOver) {
        ...
        if (thirdPerson) {
            if (joystickActive && joystickDirection.length() > 0) {
                // Usar la orientación de la cámara para el movimiento en tercera persona
                const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(camera.quaternion);
                const right = new THREE.Vector3(1, 0, 0).applyQuaternion(camera.quaternion);
                // Invertir joystickDirection.x para corregir la inversión de los controles
                direction.addScaledVector(forward, joystickDirection.y);
                direction.addScaledVector(right, -joystickDirection.x); // Invertir X
                direction.y = 0; // Restringir al plano XZ
                isMoving = true;
            }
        } else {
        ...
        if (isMoving) {
            ...
            const speed = keys[' '] || joystickDirection.length() > 0.7 ? 0.6 : 0.3;
        ...
        if (isTransitioning) {
            ...
            camera.quaternion.slerp(startQuaternion, targetQuaternion, transitionProgress);
            ...
            if (useDeviceOrientation) {
                ...
                const lookAtPoint = new THREE.Vector3(
                    player.position.x - Math.sin(firstPersonCameraYaw) * 5,
                    player.position.y + firstPersonCameraHeight + Math.sin(firstPersonCameraPitch) * 5,
                    player.position.z - Math.cos(firstPersonCameraYaw) * 5
                );
                camera.lookAt(lookAtPoint);
            }
        ...
        if (goalMarker && player && player.position.distanceTo(goalMarker.position) < 2) {
            gameWon = true;
            isPaused = true;
            document.getElementById('status').textContent = level === 5 ? `🎉 ¡Completaste el juego!` : `Nivel ${level} completado!`;
